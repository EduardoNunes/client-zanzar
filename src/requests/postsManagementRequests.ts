...
export const getPosts24hCountReq = async () => {
    const token = Cookies.get("access_token");
...
export const getPosts7dCountReq = async () => {
    const token = Cookies.get("access_token");
    ...
    try {
        ...
        const errorMessage =
            error.response?.data?.message || "Erro ao buscar posts dos últimos 7 dias.";
        toast.error(errorMessage);
        throw new Error(errorMessage);
    }
};

export const getPosts30dCountReq = async () => {
    const token = Cookies.get("access_token");
    ...
    try {
        ...
        const errorMessage =
            error.response?.data?.message || "Erro ao buscar posts dos últimos 30 dias.";
        toast.error(errorMessage);
        throw new Error(errorMessage);
    }
};

export const getAllPostsReq = async (page: number = 1): Promise<PaginatedResponse> => {
    const token = Cookies.get("access_token");
