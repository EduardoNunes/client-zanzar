export const colorNames: { [key: string]: string } = {
    // Neutros
    "#000000": "Preto",
    "#ffffff": "Branco",
    "#808080": "Cinza",
    "#c0c0c0": "Prata",
    "#a9a9a9": "Cinza Escuro",
    "#d3d3d3": "Cinza Claro",
    "#f5f5f5": "Cinza Muito Claro",
    "#2f4f4f": "Cinza Chumbo",
    "#f5f5dc": "Bege",
    "#ffe4c4": "Bege Claro",

    // Primárias e Secundárias
    "#ff0000": "Vermelho",
    "#00ff00": "Verde",
    "#0000ff": "Azul",
    "#ffff00": "Amarelo",
    "#00ffff": "Ciano",
    "#ff00ff": "Magenta",
    "#ffa500": "Laranja",
    "#800080": "Roxo",
    "#008000": "Verde Escuro",
    "#add8e6": "Azul Claro",

    // Metálicos
    "#ffd700": "Dourado",
    "#daa520": "Dourado Escuro",
    "#cd7f32": "Bronze",
    "#b87333": "Cobre",

    // Pastéis
    "#ffb6c1": "Rosa Pastel",
    "#ffefd5": "Creme",
    "#e0ffff": "Azul Pastel",
    "#ffe4e1": "Rosa Chá",
    "#fafad2": "Amarelo Pastel",
    "#f0fff0": "Verde Hortelã",

    // Tons de moda e estilo
    "#fa8072": "Salmão",
    "#dc143c": "Carmesim",
    "#4169e1": "Azul Royal",
    "#ff69b4": "Rosa-Choque",
    "#40e0d0": "Turquesa",
    "#6a5acd": "Azul Púrpura",
    "#00ced1": "Turquesa Escuro",
    "#9370db": "Violeta Médio",
    "#ba55d3": "Orquídea Médio",
    "#ff4500": "Laranja Forte",
    "#e9967a": "Salmão Claro",
    "#48d1cc": "Turquesa Médio",
    "#deb887": "Marrom Claro",
    "#a52a2a": "Marrom Escuro",
    "#8b4513": "Marrom Madeira",

    // Naturais (tons de terra)
    "#654321": "Terra",
    "#d2b48c": "Cáqui",
    "#bc8f8f": "Rosa Antigo",
    "#8b0000": "Bordô",
    "#556b2f": "Verde Musgo",
    "#808000": "Verde Oliva",
    "#6b8e23": "Verde Oliva Claro",

    // Azuis variados
    "#4682b4": "Azul Aço",
    "#5f9ea0": "Azul Cadete",
    "#b0c4de": "Azul Lavanda",
    "#87ceeb": "Azul Céu",
    "#87cefa": "Azul Céu Claro",

    // Tons de roxo/lilás
    "#9932cc": "Ameixa",
    "#8a2be2": "Azul Violeta",
    "#d8bfd8": "Lilás",
    "#dda0dd": "Orquídea",
    "#ee82ee": "Violeta Claro",

    // Extras (moda, tecnologia, infantil)
    "#ffdab9": "Pêssego",
    "#ffe4b5": "Milho Claro",
    "#fffacd": "Amarelo Pálido",
    "#ffdead": "Damasco",
    "#7fffd4": "Água-Marinha",
    "#00fa9a": "Verde Primavera",
    "#adff2f": "Verde Lima",
    "#7cfc00": "Verde Grama",
    "#32cd32": "Verde Lima Forte",
    "#228b22": "Verde Floresta",
    "#2e8b57": "Verde Mar Profundo",
    "#3cb371": "Verde Médio",

    // Tons de vinho e rosa
    "#c71585": "Violeta Escuro",
    "#db7093": "Rosa Violeta",
    "#b22222": "Vermelho Tijolo",
    "#ff6347": "Tomate",

    // Outros clássicos
    "#f08080": "Salmão Suave",
    "#20b2aa": "Verde Água Marinha",
    "#778899": "Cinza Ardósia Claro",
    "#708090": "Cinza Ardósia",
};